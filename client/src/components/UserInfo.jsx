...
const onSubmit = async (values, { setSubmitting, setStatus }) => {
  try {
    const response = await axios.get('https://toysrestapi.cyclic.app/users/userInfo', {
      headers: {
    ...
    setStatus({
      success: false,
      message: error.response.data.msg || error.response.data.err || response.message
    })
  }
...
const UserInfo = () => (
  <Formik initialValues={initialValues} validate={validate} onSubmit={onSubmit}>
    {({ isSubmitting, status }) => (
      <Form data-aos='fade-up' data-aos-duration='800' className='form-group pb-5'>
        {' '}
        <div id='quickstart' className='container py-5'>
          ...
          <div className='row'>
            <div className='col-lg-4 col-md-6 col-sm-8 mx-auto'>
              ...
              {status && status.success && <div className='text-success'>Correct data</div>}
              {status && !status.success && (
                <div className='text-danger'>Failed to recognize your email / password {status.message}</div>
              )}
            </div>
          </div>
